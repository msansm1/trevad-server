...
public class UsertvDAO extends Dao {
    ...
    public List<Usertv> getUsertvs() {
        return em.createQuery("from Usertv", Usertv.class).getResultList();
    }
    ...
    public Usertv getUsertv(Integer tvId, Integer userId) {
        TypedQuery<Usertv> q = em.createQuery("from Usertv " + "WHERE id.tvshow=:param1 AND id.user=:param2",
                Usertv.class);
    ...
    public Usertv getUsertv(Integer tvId, String token) {
        TypedQuery<Usertv> q = em.createQuery("from Usertv " + "WHERE id.tvshow=:param1 AND userBean.token=:param2",
                Usertv.class);
        q.setParameter("param1", tvId);

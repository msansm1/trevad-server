...
public class ArtistDAO extends Dao {
    ...
    public List<Artist> getArtistsForAlbum() {
        return em.createQuery("from Artist a WHERE a.artisttype.id='1'", Artist.class).getResultList();
    }

    public List<Artist> getArtistsForBook() {
        return em.createQuery("from Artist a WHERE a.artisttype.id='2'", Artist.class).getResultList();
    }

    public List<Artist> getArtistsForMovie() {
        return em.createQuery("from Artist a WHERE a.artisttype.id IN ('3','4','5')", Artist.class).getResultList();
    }
    ...
    public Artist findArtistByName(String artist) {
        List<Artist> la = em.createQuery("from Artist where name=:name", Artist.class).setParameter("name", artist)
                .getResultList();
        if (la.isEmpty()) {
            return null;
        } else {

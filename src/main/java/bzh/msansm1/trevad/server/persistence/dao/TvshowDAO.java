...
public class TvshowDAO extends Dao {
    ...
    public List<JsonShow> getUsersTvshows(Integer id) {
        TypedQuery<JsonShow> q = em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.tvshow.JsonShow(tv.id, tv.title, "
                        + "tv.cover, tv.description)" + "from Tvshow tv, Usertv ut, User u "
                        + "where tv.id=ut.id.tvshow and ut.id.user=u.id and u.id=:param1", JsonShow.class);
        q.setParameter("param1", id);
        return q.getResultList();
    }

    public List<Tvshow> getTvshowsForList(int index, int limit, String orderBy, String orderDir) {
        String dir = "DESC";
        if (orderDir != null) {
            dir = orderDir;
        }
        return em.createQuery("FROM Tvshow ORDER BY " + orderBy + " " + dir, Tvshow.class).setFirstResult(index)
                .setMaxResults(limit).getResultList();
    }

    public SerieStats getUserStats(Integer userId) {
        TypedQuery<SerieStats> q = em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.home.SerieStats(COUNT(ut.tvshowBean.id))"
                        + "from Usertv ut " + "where ut.id.user=:userId", SerieStats.class);
        q.setParameter("userId", userId);
        return q.getSingleResult();
    }

    public List<JsonShow> getUserTvshows(int index, int limit, String orderBy, String orderDir, Integer userId) {
        String dir = "DESC";
        if (orderDir != null) {
            dir = orderDir;
        }
        return em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.tvshow.JsonShow(t.id, t.title, "
                        ...
                        + "t.isseriedone, true, ut.rating) " + "FROM Tvshow t INNER JOIN t.usertvs ut "

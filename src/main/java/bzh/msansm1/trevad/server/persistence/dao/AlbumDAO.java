...
public class AlbumDAO extends Dao {
    ...
    public List<JsonAlbum> getUsersAlbums(Integer id) {
        TypedQuery<JsonAlbum> q = em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.album.JsonAlbum(a.id, a.title, " + "a.cover)"
    ...
    public List<JsonAlbum> getUserAlbumsForList(int index, int limit, String orderBy, String orderDir, Integer userId) {
        ...
        return em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.album.JsonAlbum(a.id, a.title, "
    ...
    public AlbumStats getUserStats(Integer userId) {
        TypedQuery<AlbumStats> q = em
                .createQuery("SELECT NEW bzh.msansm1.trevad.server.json.home.AlbumStats(COUNT(ua.albumBean.id))"
